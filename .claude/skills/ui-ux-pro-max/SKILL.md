...
## Precedence: defer to `ui-styling` for brand identity

If the `ui-styling` skill (Claude/Anthropic design system — cream canvas, coral accent, Copernicus/Tiempos display, StyreneB/Inter body) applies to the current work, **its rules win** for anything brand-identity-related:

- Color palette, accent colors, semantic color tokens → `ui-styling`, not §4/§6 (`Style Selection`, `Typography & Color`) palette/font recommendations below.
- Typography choices (display/body font pairing) → `ui-styling`, not the `--domain typography` search or §6 font-pairing guidance.
- Overall visual style (cream/coral/dark-navy editorial system) → `ui-styling`, not `--design-system` style recommendations.

Still use **this skill** for everything `ui-styling` doesn't cover: accessibility (§1), touch/interaction (§2), performance (§3), layout/responsive (§5), animation (§7), forms/feedback (§8), navigation (§9), and charts/data (§10) — plus stack-specific implementation guidance (Step 4). When running `--design-system`, treat its style/color/typography output as a fallback only for surfaces `ui-styling` doesn't define, never as an override of it.
