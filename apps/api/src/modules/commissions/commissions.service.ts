...
import { PaginatedCommissionsDto } from './dto/commission-query.dto';
import {
...
import { CommissionStatusHistoryDto } from './dto/commission-history.dto';
import { CommissionQueueDto } from './dto/commission-queue.dto';
import { CommissionNoteVisibility, CommissionStatus } from '@prisma/client';

const QUEUE_STATUSES: CommissionStatus[] = [
...
const QUEUE_STATUS_RANK = new Map(
  QUEUE_STATUSES.map((status, index) => [status, index]),
);

const COMMISSION_RECEIVED_NOTIFICATION_CONFIG_TYPE =
  'art.commissionreceived.notification';
const NEW_COMMISSION_EMAIL_TEMPLATE_CONFIG_TYPE =
  'art.commissionreceived.templateid';
const DELIVERY_EMAIL_TEMPLATE_CONFIG_TYPE =
